...
相比于 ac_and_train.py 此处采用的方法是：
    ...
    接着将不确定性策略选出的数据输入 gmm 模型，选出离各自聚类中心较近的数据，加到 train 集中，重新训练；
...
def testCustomODModel(model_path, unod_date_path, min_thresh=0.3):
    ...
    for img in images:
        ...
        if torch.cuda.is_available():
            ...
            img_tensor = torch.from_numpy(img2 / 255.).permute(2, 0, 1).float()
...
def ac_train(args):
    ...
    for epoch in range(args['epochs']):
        ...
        if args['output_dir']:
            ...
            # 2-只保存最新 epoch 的模型（直接覆盖旧模型）
            # new_model_name = 'model_{}_{}.pth'.format('custom_voc', epoch)
...
def trainer_tmp(model_path, is_train, is_first, test_model_path, valid_path,
    ...
    if not is_train:
        ...
        args['epochs'] = 5  # 不是第一次，之后的迭代训练也要保证所有类的 AP 在指定阈值之上
    ac_train(args)


def ac_and_gmm_choose(model_path, new_images_files, pca_path, M, lambda_, ret_type='txtfile'):
    """
    两种方式结合挑选数据
    ...
    sorted_data, nearest_data_near = demo.GMMClusting_opt_3th(pre_5_percent_images, pickle_model, M, K,
    ...
    # 调整 list（只保存挑选出来的图像名）
    nearest_data_list = []
    for k in nearest_data_near:
        print(k.name, k.score, k.label)
        nearest_data_list.append(k.name)
    print('after GMM choose, there has:', len(nearest_data_list), 'images!')

    if ret_type == 'txtfile':
        # # 将选出的图片数据存入文件
        import time
        txt_file = 'pre_5_percent_images_ac_then_gmm_' + str(time.time()).split('.')[0] + '.txt'
        filetools.text_save(txt_file, nearest_data_list)
        return txt_file
    if ret_type == 'list':
        # 仅返回挑选出的图片列表，而不保存成文件
        return nearest_data_list


if __name__ == '__main__':
    # 训练 或 测试
    # train_data_path = r"/home/jiangb/D2_acl_tl/military_ac/only_ac/train"   # 训练集
    # test_data_path = r"/home/jiangb/D2_acl_tl/military_ac/only_ac/test"     # 测试集
    # # train_data_path = r"/home/jiang/Dataset/MilitaryOB_5_class_torch_3th/train"  # 训练集
    # # test_data_path = r"/home/jiang/Dataset/MilitaryOB_5_class_torch_3th/test"  # 测试集
    # # model_path = 'ac_then_gmm_choose/8/model_custom_voc_3_loss_0.13337570428848267.pth'     # 当前轮使用的初始模型
    # model_path = 'acl_tl/military/only_ac/ac_then_gmm/4/model_custom_voc_4_loss_0.06559343636035919.pth'
    # output_dir = './checkpoints/acl_tl/military/only_ac/ac_then_gmm/5'  # 模型输出路径
    ...
    # 先用不确定性策略挑选出 2M 张图片（这其中包含 hard sample），再用 GMM 聚类选出靠近聚类中心的前 M 张图片
    # M = 130
    # model_name = 'model_custom_voc_4_loss_0.06559343636035919.pth'
    # model_path = os.path.join(os.getcwd(), 'checkpoints', 'acl_tl', 'military', 'only_ac', 'ac_then_gmm', '4', model_name)
    # choose_file = r'/home/jiangb/D2_acl_tl/military_ac/only_ac/unlabelpool/JPEGImages'
    # # pca_path 是测试数据集的路径（因为要选择与测试集数据分布靠近的数据）
    # pca_path = r'/home/jiangb/D2_acl_tl/military_ac/only_ac/test/JPEGImages'
    ...
    # to_file_path = r"/home/jiangb/D2_acl_tl/military_ac/only_ac/unlabelpool/tmp"
    ...
    test_data_path = r"/home/jiangb/D2_acl_tl/military_ac/only_ac/test"  # 测试集
    model_path = 'xxx'
    output_dir = 'xxx'  # 模型输出路径
    test_model_path = r'acl_tl/military/only_ac/ac_then_gmm/2/model_custom_voc_4_loss_0.2919769287109375.pth'  # 用于测试的模型路径
    valid_path = r'/home/jiangb/D2_acl_tl/military_ac/watercolor_atl'  # 验证集的路径（路径不能包含中文）
    is_train = False  # 是训练还是测试
    is_first = False  # 是否是初始训练轮次
    trainer_tmp(model_path, is_train, is_first, test_model_path, valid_path,
                train_data_path, test_data_path, output_dir)  # 记得去 函数内去改参数
