...
from voc_eval import _write_voc_results_file, _do_python_eval, _do_python_eval_custom_voc, \
    _write_custom_voc_results_file
import utils
...
def train_one_epoch(model, optimizer, data_loader, device, epoch, print_freq):
    ...
    loss_all_now = None
    lr_scheduler = None
    # 先暂时注释掉，不让在 epoch 0 中逐渐递增 lr，直接从自己指定的 lr 开始训练
    # if epoch == 0:
...
@torch.no_grad()
def custom_voc_evaluate(model, data_loader, device, CLASS, thresh=0.8):
    """
    该函数只会在 ac_train.py 中的训练和测试过程被调用，会在 output 文件夹中产出目前模型的检测图像（记得指定 bbox 阈值）；
    而 ac.py 中的并不包含测试过程，而仅是利用模型对图片进行检测，所以不会调用此函数，其中有自己的产出检测图像的函数（可在那边指定阈值）
    :param thresh: 检测框的阈值
    :param model: 测试的模型
    :param data_loader: 训练数据加载
    ...
    all_boxes = [[] for i in range(len(CLASS))]
    image_index = []
    for image, targets in metric_logger.log_every(data_loader, print_freq=100, header=header):
        ...
        image_boxes = [[] for i in range(len(CLASS))]  # 需要修改该值
        for o in outputs:
            for i in range(o['boxes'].shape[0]):
                image_boxes[o['labels'][i]].extend([
                    torch.cat([o['boxes'][i], o['scores'][i].unsqueeze(0)], dim=0)
    ...
    if utils.is_main_process():
        ...
        for iig in image_index_gathered:
            image_index += iig
        # 这里可以指定要将阈值超过多少的检测框画到图像中去（默认是0.8）
        # 会在 output 文件夹下产生检测后的图片以及将 tmp 文件夹清空（若不需要，请自行去函数内选择性注释）
        # print(image_index)
        _write_custom_voc_results_file(data_loader, all_boxes, image_index, data_loader.dataset.root, CLASS, thread=thresh)
        # 会在 tmp 文件夹下生成相关检测参数文件，以及计算各类 ap
        mAP, retStr = _do_python_eval_custom_voc(data_loader, CLASS)
    torch.set_num_threads(n_threads)
    return mAP, retStr
